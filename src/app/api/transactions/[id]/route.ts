...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  const oldTran = await Transaction.findOne({ _id: params.id, userId: session.user.id });
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  ...
  const tran = await Transaction.findOneAndDelete({ _id: params.id, userId: session.user.id });
